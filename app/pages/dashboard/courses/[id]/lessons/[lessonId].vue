...
const slides = computed<Slide[]>(() => {
  ...
  return [
    { kind: 'intro' },
    ...lesson.value.content.steps.map((step, i) => ({ kind: 'step' as const, step, stepIndex: i })),
    { kind: 'summary' },
  ]
})
...
const progressPercent = computed(() =>
  slides.value.length > 1
    ? Math.round((currentIndex.value / (slides.value.length - 1)) * 100)
    : 0,
)
</script>
      ...
      <div class="flex-1 flex flex-col max-w-2xl mx-auto w-full px-4 py-10">
        ...
        <template v-if="currentSlide?.kind === 'intro'">
          <div class="flex-1 space-y-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-emerald-400">Lesson {{ lesson.lesson_number }}</p>
        ...
        <template v-else-if="currentSlide?.kind === 'summary'">
          <div class="flex-1 space-y-6">
            ...
            <div
              ...
              :class="scorePercent >= 70 ? 'border-emerald-700/50 bg-emerald-900/10' : 'border-amber-700/50 bg-amber-900/10'"
            >
              <p class="text-3xl font-bold" :class="scorePercent >= 70 ? 'text-emerald-400' : 'text-amber-400'">
                {{ scorePercent }}%
              </p>
              <p class="text-slate-300 text-sm">{{ correctCount }} correct out of {{ answeredCount }} exercises</p>
              <p v-if="scorePercent >= 70" class="text-emerald-400 text-sm font-medium">Great work! Lesson complete.</p>
              <p v-else class="text-amber-400 text-sm font-medium">Keep practicing — you can review by retaking the lesson.</p>
            </div>
