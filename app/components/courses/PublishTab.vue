<script setup lang="ts">
  ...
  async function handleGenerate() {
    ...
    try {
      const updated = await $fetch<Course>(`/api/courses/${props.courseId}/generate`, {
        method: 'POST'
      });
  ...
  async function handleSubmitReview() {
    ...
    try {
      const updated = await $fetch<Course>(`/api/courses/${props.courseId}/submit-review`, {
        method: 'POST'
      });
      if (props.course) emit('update:course', { ...props.course, ...updated });
    } catch (err: any) {
  ...
  <div class="space-y-8">
    ...
    <div class="space-y-4">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-400">
          The AI reads your uploaded PDFs and builds the full course structure: modules, lessons,
          objectives, and key topics. Once the structure is ready, you can review and edit it before
          publishing.
        </p>
      </div>
      ...
      <div
        ...
        class="rounded-xl border border-amber-500/40 bg-amber-950/30 p-4 space-y-3"
      >
        <p class="text-sm font-medium text-amber-300">Re-generate structure?</p>
        <p class="text-sm text-amber-200/70">
          This will discard the current course structure (all modules and lessons) and generate a
          new one from your PDFs. This action cannot be undone.
        </p>
    ...
    <div class="space-y-4">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-400">
          Once the course structure is generated and you're happy with it, submit it for admin
          review. After approval, your course will be published and available to learners.
        </p>
      </div>
      ...
      <div
        ...
        class="rounded-xl border border-amber-500/30 bg-amber-950/20 p-4"
      >
        <p class="text-sm text-amber-300/80">
          Your course is currently under review. You'll be notified once an admin has evaluated it.
        </p>
      </div>

      <div
        v-if="course?.status === 'approved'"
        class="rounded-xl border border-emerald-500/30 bg-emerald-950/20 p-4"
      >
      ...
      <p
        v-else-if="!isReady && course?.status !== 'pending_review' && course?.status !== 'approved'"
