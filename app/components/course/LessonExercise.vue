...
<template>
  <div class="rounded-xl border bg-slate-900 p-5 space-y-4" :class="isSubmitted ? (isCorrect ? 'border-emerald-700/50' : 'border-red-700/50') : 'border-slate-800'">
    <!-- Exercise number -->
    ...
    <template v-if="exercise.type === 'multiple_choice'">
      ...
      <div class="space-y-2">
        <button
          ...
          :class="{
            'border-slate-700 text-slate-300 hover:border-slate-500 hover:bg-slate-800/50': !isSubmitted && selectedOption !== i,
            'border-emerald-500 bg-emerald-900/30 text-emerald-300': isSubmitted && i === exercise.correct_index,
            'border-red-500 bg-red-900/30 text-red-300': isSubmitted && selectedOption === i && i !== exercise.correct_index,
            'border-slate-700 text-slate-500': isSubmitted && i !== exercise.correct_index && selectedOption !== i,
          }"
    ...
    <template v-else-if="exercise.type === 'true_false'">
      ...
      <div class="flex gap-3">
        <button
          ...
          :class="{
            'border-slate-700 text-slate-300 hover:border-emerald-600 hover:bg-emerald-900/20 hover:text-emerald-300': !isSubmitted,
            'border-emerald-500 bg-emerald-900/30 text-emerald-300': isSubmitted && exercise.is_true,
            'border-red-500 bg-red-900/30 text-red-300': isSubmitted && selectedBool === true && !exercise.is_true,
            'border-slate-700 text-slate-500': isSubmitted && selectedBool !== true && !exercise.is_true,
          }"
          :disabled="isSubmitted"
        ...
        <button
          class="flex-1 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors"
          :class="{
            'border-slate-700 text-slate-300 hover:border-red-600 hover:bg-red-900/20 hover:text-red-300': !isSubmitted,
            'border-emerald-500 bg-emerald-900/30 text-emerald-300': isSubmitted && !exercise.is_true,
            'border-red-500 bg-red-900/30 text-red-300': isSubmitted && selectedBool === false && exercise.is_true,
            'border-slate-700 text-slate-500': isSubmitted && selectedBool !== false && exercise.is_true,
          }"
          :disabled="isSubmitted"
    ...
    <!-- ─── Feedback ─── -->
    <div v-if="isSubmitted" class="rounded-lg px-4 py-3 text-sm" :class="isCorrect ? 'bg-emerald-900/20 border border-emerald-800/40 text-emerald-300' : 'bg-red-900/20 border border-red-800/40 text-red-300'">
      <div class="flex items-start gap-2">
        <svg v-if="isCorrect" class="w-4 h-4 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
