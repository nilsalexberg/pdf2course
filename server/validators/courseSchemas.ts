...
export const courseCreateSchema = z.object({
  ...
  numModules: z.coerce
    ...
    .refine(
      ...
      `numModules must be between ${CourseCreateLimits.numModules.min} and ${CourseCreateLimits.numModules.max}`
    ),
  lessonsPerModule: z.coerce
    .number()
    .int()
    .refine(
      (v) =>
      ...
      `lessonsPerModule must be between ${CourseCreateLimits.lessonsPerModule.min} and ${CourseCreateLimits.lessonsPerModule.max}`
    ),
  languageLevel: z.enum(COURSE_languageLevelS as unknown as [string, ...string[]]),
